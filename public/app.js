...
function statusLabel(status) {
  return ({ OVERDUE: "Overdue", DUE_SOON: "Due soon", OK: "On schedule", NOT_SET: "Not set" })[status] || status;
}
...
function renderDashboard(summary) {
  ...
  Object.entries(map).forEach(([id, value]) => { $(`#${id}`).textContent = formatNumber(value); });
}
...
async function loadAll({ notify = false } = {}) {
  ...
  try {
    ...
    renderStatusTable();
    setConnection(true);
    elements.lastUpdated.textContent = `Updated ${new Intl.DateTimeFormat(undefined, { hour: "2-digit", minute: "2-digit" }).format(new Date())}`;
...
function bindEvents() {
  ...
  elements.recentLimit.addEventListener("change", async () => {
    try { await loadRecent(); } catch (error) { showAlert(`Unable to load recent records: ${error.message}`); }
  });
  ...
  window.addEventListener("offline", () => { setConnection(false); showAlert("The browser is offline. Previously loaded information remains visible."); });
}
