 ...
 * Optional: ?limit=20
 * Returns recent completed service records. Genset, project and
 ...
 * Body: { service_schedule_id, service_date, engine_hours?,
 *         performed_by?, work_order_number?, report_reference?, remarks? }
 *
 * The record is linked to a service schedule. Genset and service item
 * are no longer stored directly on the record; they are derived through
 * the schedule.
 */
router.post("/", async (req, res, next) => {
  try {
    ...
    // Validate the schedule exists before inserting.
